...
//供关闭线程时等待
{
	...
	{
		ObReferenceObjectByHandle(hThread, NULL, KernelMode, NULL, &pDeviceKeyboardDeviceExtension->pThreadObj, NULL);
	}
	ZwClose(hThread);
	return Status;
}
VOID WriteFileThread(PVOID Context)
{
	...
	while (true)
	{
		KeWaitForSingleObject(&pKeyboardDeviceExtension->sem, NULL, KernelMode, false, NULL);
		if (pKeyboardDeviceExtension->bThreadTerminate == true)
		{
			PsTerminateSystemThread(STATUS_SUCCESS);
		}
		pkData = CONTAINING_RECORD(pKeyboardDeviceExtension->MessageList, KEY_DATA, ListEntry);
